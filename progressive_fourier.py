...
def fourier_transform_animation(image_path, points_per_iteration=10):
    ...
    points = [(random.randint(0, cols-1), random.randint(0, rows-1)) for _ in range(num_points)]
    random.shuffle(points)  # Shuffle the points for randomness

    # Animation loop
    for i in range(0, len(points), points_per_iteration):
        upper_bound = min(i + points_per_iteration, len(points))

        for point in points[i:upper_bound]:
